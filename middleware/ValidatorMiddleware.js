...
export const validateVenue = withValidationError([
  ...
  body("location.pincode").notEmpty().withMessage("Pincode is required").trim(),
  ...
  body("capacity.max")
    ...
    .custom((value, { req }) => {
      if (value < req.body.capacity.min) {
        throw new Error(
  ...
  body("availableFor")
    .isArray()
    .withMessage("Available for must be an array")
    .notEmpty()
...
export const validateDecoration = withValidationError(
  [
    ...
    body("dimensions.minSpace")
      .notEmpty()
      .withMessage("Minimum space requirement is required"),
  ],
  (req, res, next) => {
...
export const validateRentalItem = withValidationError([
  ...
  body("rentalStartDate")
    ...
    .isISO8601()
    .withMessage("Rental start date must be a valid ISO 8601 date (e.g., YYYY-MM-DD)"),
